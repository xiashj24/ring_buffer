 ...
 *      v6, which is essentially the same as https://rigtorp.se/ringbuffer/
 * @note use only 2 cache lines instead of 4
 * @note std::hardware_destructive_interference_size is most likely 64 bytes on x86 and 128 bytes on ARM
 */
...
public:
    ...
    bool try_push(const T &value) noexcept(std::is_nothrow_copy_assignable_v<T>)
    {
        ...
        {
            cached_head_ = head_.load(std::memory_order_acquire); // reload
    ...
    bool try_pop(T &value) noexcept(std::is_nothrow_copy_assignable_v<T>)
    {
        ...
        {
            cached_tail_ = tail_.load(std::memory_order_acquire); // reload
